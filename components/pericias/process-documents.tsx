...
/**
 * Documentos do processo — lista + botão "Analisar com IA".
 *
 * A IA seleciona automaticamente os docs mais relevantes,
 * analisa server-side, e marca quais foram usados.
 * O perito pode selecionar manualmente se preferir.
 */
...
const STATUS_CONFIG: Record<string, { icon: typeof Check; label: string; className: string }> = {
  downloaded: { icon: Check, label: 'Disponível', className: 'text-[#4d7c0f] bg-lime-50 border-lime-200' },
  ...
  unavailable:{ icon: X,     label: 'Indisponível', className: 'text-slate-400 bg-slate-50 border-slate-200' },
}
...
export function ProcessDocuments({ periciaId, attachments, docsUsadosNaAnalise = [] }: Props) {
  ...
  const loading = syncLoading || iaLoading
  ...
  function handleAnaliseIA() {
    // Se tem seleção manual, usa esses IDs; senão IA escolhe automaticamente
    const ids = modoSelecao && selected.size > 0 ? [...selected] : undefined
    startIaTransition(async () => {
      ...
      try {
        ...
        const data = await res.json()
        setResult({ ok: data.ok, message: data.message })
        if (data.ok) { setSelected(new Set()); setModoSelecao(false); router.refresh() }
      ...
      <div className="flex items-center gap-4">
        ...
        {attachments.length > 0 && (
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 text-[9px] font-bold uppercase tracking-widest hover:bg-slate-800 transition-colors disabled:opacity-30"
          >
            {iaLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
            {iaLoading ? 'Analisando...' : modoSelecao && selected.size > 0 ? `Analisar ${selected.size} selecionados` : 'Analisar com IA'}
          </button>
        )}
      </div>
        ...
        {attachments.length === 0 ? (
          <div className="px-8 py-12 text-center space-y-4">
            ...
            <button
              onClick={handleSync}
              disabled={loading}
              className="inline-flex items-center gap-2 bg-[#a3e635] hover:bg-[#bef264] px-5 py-3 text-[10px] font-black uppercase tracking-[0.2em] text-slate-900 transition-all disabled:opacity-30"
            >
          ...
          <>
            ...
            <div className="px-6 py-2 border-b border-slate-100 flex items-center justify-between">
              <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest">
                ...
                {usadosSet.size > 0 && ` · ${usadosSet.size} usado${usadosSet.size !== 1 ? 's' : ''} na análise`}
              </span>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => { setModoSelecao(!modoSelecao); setSelected(new Set()) }}
                  className={cn(
                    'text-[9px] font-bold uppercase tracking-widest transition-colors',
                    modoSelecao ? 'text-slate-900' : 'text-slate-400 hover:text-slate-700',
                  )}
                >
                  {modoSelecao ? 'Cancelar seleção' : 'Selecionar docs'}
                </button>
                <button
                  onClick={handleSync}
                  disabled={loading}
                  className="flex items-center gap-1 text-[9px] font-bold uppercase tracking-widest text-slate-400 hover:text-slate-700 transition-colors disabled:opacity-30"
                >
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isSelected ? 'bg-lime-50' : modoSelecao ? 'hover:bg-slate-50' : '',
                    )}
                    onClick={modoSelecao ? () => toggleSelect(att.id) : undefined}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-bold text-slate-900 truncate">{att.name}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        {att.type && <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest">{att.type}</span>}
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {usadoNaIA && (
                        <span className="inline-flex items-center gap-1 text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 bg-slate-900 text-white">
