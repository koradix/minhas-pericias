...
const inputCls =
  'w-full rounded-lg border border-[#e2e8f0] bg-white px-3 py-2.5 text-[14px] text-[#1f2937] focus:outline-none focus:ring-2 focus:ring-[#416900]/20 focus:border-[#416900] placeholder-[#d1d5db] transition-all'

const textareaCls =
  `${inputCls} resize-none`

const labelCls =
  'block text-[11px] font-semibold uppercase tracking-[0.1em] text-[#9ca3af] mb-1.5 font-inter'
  ...
  return (
    <section className="rounded-xl border border-[#e2e8f0] bg-white">
      <div className="flex flex-col items-center gap-4 px-6 py-12 text-center">
        <div className="flex h-12 w-12 items-center justify-center rounded-full border-2 border-[#e2e8f0] bg-[#f8f9ff]">
          <Lock className="h-6 w-6 text-[#d1d5db]" />
        </div>
        <div>
          <p className="text-[15px] font-semibold text-[#374151] font-manrope">
...
function TemplateSelector({
  ...
  return (
    <div className="space-y-3">
      ...
      <label className="flex items-center gap-3 cursor-pointer rounded-lg border px-4 py-3 transition-all hover:bg-[#f8f9ff]
        border-[#e2e8f0] has-[:checked]:border-[#416900] has-[:checked]:bg-[#f4fce3]">
        <input
          type="radio"
          name="template"
          value=""
          ...
          className="accent-[#416900]"
        />
        <div className="min-w-0">
          <p className="text-[14px] font-semibold text-[#1f2937] font-manrope">Template padrão Perilab</p>
          <p className="text-[12px] text-[#9ca3af] font-inter">DOCX gerado automaticamente, sem timbrado</p>
        </div>
      </label>

      {/* Existing templates */}
      {templates.map((t) => (
        <label key={t.id} className="flex items-center gap-3 cursor-pointer rounded-lg border px-4 py-3 transition-all hover:bg-[#f8f9ff]
          border-[#e2e8f0] has-[:checked]:border-[#416900] has-[:checked]:bg-[#f4fce3]">
          <input
            type="radio"
            name="template"
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[14px] font-semibold text-[#1f2937] font-manrope truncate">{t.nome}</p>
            <p className="text-[12px] text-[#9ca3af] font-inter truncate">{t.nomeArquivo}</p>
            {t.tagsDetected.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1.5">
                {t.tagsDetected.slice(0, 4).map((tag) => (
                  <span key={tag} className="text-[10px] bg-[#f2f3f9] text-[#6b7280] rounded px-1.5 py-0.5 font-mono">{tag}</span>
                ))}
                {t.tagsDetected.length > 4 && (
                  <span className="text-[10px] text-[#9ca3af]">+{t.tagsDetected.length - 4}</span>
                )}
              </div>
            )}
          </div>
          <button
            ...
            className="p-1 text-[#d1d5db] hover:text-rose-500 transition-colors flex-shrink-0"
          >
      ...
      {!showNomeInput && (
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg border border-dashed border-[#e2e8f0] hover:border-[#416900] hover:bg-[#f4fce3] px-4 py-3 text-[13px] font-medium text-[#6b7280] hover:text-[#416900] transition-all disabled:opacity-50"
        >
          {isUploading
      ...
      {showNomeInput && (
        <div className="rounded-lg border border-[#e2e8f0] bg-[#f8f9ff] p-4 space-y-3">
          <p className="text-[13px] font-semibold text-[#1f2937] font-manrope truncate">
            {pendingFile?.name}
          </p>
          <div>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-[#416900] hover:bg-[#84cc16] hover:text-[#102000] px-3 py-2 text-[13px] font-semibold text-white transition-all disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-[#e2e8f0] px-3 py-2 text-[13px] font-medium text-[#6b7280] hover:bg-white transition-all"
            >
  ...
  return (
    <div className="rounded-xl border border-[#e2e8f0] bg-white">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between px-6 py-4 hover:bg-[#f8f9ff] transition-colors"
      >
        <div className="flex items-center gap-2.5">
          <Clock className="h-4 w-4 text-[#9ca3af]" />
          <p className="text-[14px] font-semibold text-[#374151] font-manrope">
            Histórico de versões
            <span className="ml-2 text-[12px] font-normal text-[#9ca3af]">({versoes.length})</span>
          </p>
        </div>
        ...
        <div className="divide-y divide-[#f2f3f9] border-t border-[#f2f3f9]">
          {versoes.map((v) => (
            <div key={v.id} className="px-6 py-4 flex items-center gap-3">
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#f2f3f9] text-[12px] font-bold text-[#6b7280] flex-shrink-0">
                v{v.versao}
              </span>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-medium text-[#374151] font-inter">
              ...
              {v.snapshot.valorHonorarios != null && (
                <span className="text-[13px] font-semibold text-[#416900]">
    ...
    <div className="space-y-5">
        ...
        <>
          <section className="rounded-xl border border-[#e2e8f0] bg-white">
            <div className="flex items-center gap-3 px-6 py-5 border-b border-[#f2f3f9]">
              <ClipboardList className="h-5 w-5 text-[#416900]" />
              <h2 className="text-[15px] font-semibold text-[#1f2937] font-manrope">Modelo do documento</h2>
            </div>
            <div className="px-6 py-5">
              <p className="text-[13px] text-[#6b7280] mb-4 font-inter">
                Selecione um modelo existente ou suba seu próprio .docx com timbrado.
                A IA vai preencher as tags <code className="bg-[#f2f3f9] rounded px-1 text-[12px]">{'{{'}</code><code className="bg-[#f2f3f9] rounded px-1 text-[12px]">{'}}'}</code> automaticamente.
              </p>
              <TemplateSelector
                templates={templates}
          ...
          <section className="rounded-xl border border-[#e2e8f0] bg-white">
            <div className="flex items-center gap-3 px-6 py-5 border-b border-[#f2f3f9]">
              <h2 className="text-[15px] font-semibold text-[#1f2937] font-manrope">Dados do perito</h2>
            </div>
          ...
          {saveError && (
            <div className="flex items-center gap-2 rounded-lg bg-rose-50 border border-rose-100 px-4 py-3">
              ...
              <p className="text-[13px] text-rose-700 font-inter">{saveError}</p>
            </div>
          )}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 rounded-lg bg-[#416900] hover:bg-[#84cc16] hover:text-[#102000] px-4 py-3 text-[15px] font-semibold text-white transition-all disabled:opacity-50"
          >
            {isGenerating
          ...
          {genError && (
            <div className="flex items-center gap-2 rounded-lg bg-rose-50 border border-rose-100 px-4 py-3">
              <AlertCircle className="h-4 w-4 text-rose-500 flex-shrink-0" />
              <p className="text-[13px] text-rose-700 font-inter">{genError}</p>
            </div>
          )}
        </>
      )}
      ...
      {phase === 'form' && (
        <>
          {/* AI badge */}
          {lastIaModel && (
            <div className="flex items-center gap-2 rounded-lg bg-[#f4fce3] border border-[#d8f5a2] px-4 py-2.5">
              <CheckCircle2 className="h-4 w-4 text-[#416900] flex-shrink-0" />
              <p className="text-[13px] font-semibold text-[#416900] font-inter">
                Proposta gerada via {lastIaModel} — revise e ajuste antes de salvar
              </p>
            </div>
          )}
          ...
          {(qaResult?.campos_faltantes?.length ?? 0) > 0 && (
            <div className="rounded-lg bg-amber-50 border border-amber-100 px-4 py-3 space-y-1">
              <p className="text-[13px] font-semibold text-amber-800 font-manrope">Campos identificados como faltantes pela IA:</p>
              {qaResult!.campos_faltantes.map((c, i) => (
                <p key={i} className="text-[12px] text-amber-700 font-inter">• {c}</p>
              ))}
            </div>
          )}

          <section className="rounded-xl border border-[#e2e8f0] bg-white">
            <div className="px-6 py-5 border-b border-[#f2f3f9] bg-[#f8f9ff]">
              <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#9ca3af]">Conteúdo da proposta</p>
            </div>
            <div className="divide-y divide-[#f2f3f9]">
              ...
              <div className="grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-[#f2f3f9]">
                <div className="px-6 py-5">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              disabled={isSaving}
              className="flex items-center gap-2 rounded-lg border border-[#e2e8f0] hover:bg-[#f8f9ff] px-4 py-2.5 text-[14px] font-medium text-[#6b7280] transition-all disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-lg bg-[#416900] hover:bg-[#84cc16] hover:text-[#102000] px-4 py-2.5 text-[14px] font-semibold text-white transition-all disabled:opacity-50"
            >
      ...
      {phase === 'result' && (
        <>
          <div className="rounded-lg border border-[#d8f5a2] bg-[#f4fce3] px-5 py-4 flex items-center gap-3">
            <CheckCircle2 className="h-5 w-5 text-[#416900] flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-[15px] font-semibold text-[#416900] font-manrope">Proposta gerada</p>
              {content.valorHonorarios != null && (
                <p className="text-[13px] text-[#416900]/70 font-inter">
          ...
          {dlError && (
            <div className="flex items-center gap-2 rounded-lg bg-rose-50 border border-rose-100 px-4 py-3">
              <AlertCircle className="h-4 w-4 text-rose-500 flex-shrink-0" />
              <p className="text-[13px] text-rose-700 font-inter">{dlError}</p>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-lg border border-[#e2e8f0] hover:bg-[#f8f9ff] px-4 py-2.5 text-[14px] font-medium text-[#6b7280] transition-all"
            >
            ...
            <div className="divide-y divide-[#f2f3f9]">
              {content.resumoTecnico && (
                <div className="px-6 py-5">
                  ...
                  <p className="text-[14px] text-[#374151] leading-relaxed whitespace-pre-line font-inter">{content.resumoTecnico}</p>
                </div>
              )}
              {content.descricaoServicos && (
                <div className="px-6 py-5">
                  <p className={labelCls}>Escopo dos serviços</p>
                  <p className="text-[14px] text-[#374151] leading-relaxed whitespace-pre-line font-inter">{content.descricaoServicos}</p>
                </div>
              )}
                ...
                <div className="grid sm:grid-cols-3 px-6 py-5 gap-4">
                  {content.valorHonorarios != null && (
                    <div>
                      ...
                      <p className="text-[15px] font-bold text-[#416900]">{formatBRL(content.valorHonorarios)}</p>
                    </div>
                  )}
                  {content.custoDeslocamento != null && (
                    <div>
                      <p className={labelCls}>Deslocamento</p>
                      <p className="text-[15px] font-semibold text-[#374151]">{formatBRL(content.custoDeslocamento)}</p>
                    </div>
                  )}
                  {content.prazoEntrega && (
                    <div>
                      <p className={labelCls}>Prazo</p>
                      <p className="text-[14px] font-semibold text-[#374151]">{content.prazoEntrega}</p>
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="px-6 py-5">
                  ...
                  <ul className="space-y-1.5 mt-1">
                    {content.riscosEPendencias!.map((r, i) => (
                      <li key={i} className="flex items-start gap-2 text-[13px] text-amber-800 font-inter">
