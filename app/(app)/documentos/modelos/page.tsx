import { BookOpen, Plus, Upload, Zap } from "lucide-react"
          ...
          <Card key={m.id} className="hover:shadow-md transition-shadow cursor-pointer group">
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={tipoColors[m.tipo] + " inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-semibold"}>
                  {tipoDocumentoLabels[m.tipo]}
                </span>
              </div>
              <CardTitle className="text-sm mt-3 group-hover:text-blue-700 transition-colors">{m.nome}</CardTitle>
            </CardHeader>
            <CardContent className="pt-0">
              <p className="text-xs text-slate-500 leading-relaxed mb-4 line-clamp-2">{m.descricao}</p>

              {m.arquivosTreinamento.length > 0 && (
                <div className="mb-4">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Arquivos de treinamento</p>
                  <div className="space-y-1">
                    {m.arquivosTreinamento.map((a) => (
                      <div key={a.id} className="flex items-center gap-2 text-xs text-slate-500">
                        <span className={a.tipo === "PDF" ? "text-red-500" : "text-blue-500"}>{a.tipo}</span>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                <span className="text-[10px] text-slate-400">{m.totalUsos} usos · criado em {m.criadoEm}</span>
