      ...
      <div className="flex flex-wrap gap-3">
        ...
        {(tipoFilter || ufFilter) && (
          <button
            ...
            className="flex items-center gap-1 h-9 px-3 rounded-lg border border-slate-200 text-xs text-slate-500 hover:bg-slate-50 transition-colors"
          >
        ...
        {isMatchMode && (
          <span className="flex items-center gap-1.5 h-9 px-3 rounded-lg bg-lime-50 border border-lime-200 text-xs font-medium text-lime-700">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className="rounded-xl border border-slate-200 bg-white p-5 hover:shadow-md transition-shadow flex flex-col gap-3"
              >
                {/* Header: avatar + nome + score */}
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-3">
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-lime-500 text-slate-900 text-sm font-bold">
                      {p.nome.split(' ').map((n) => n[0]).slice(0, 2).join('')}
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-slate-900 leading-tight">{p.nome}</p>
                ...
                <div className="flex flex-wrap gap-1">
                  {p.especialidades.map((e) => (
                    <span
                      key={e}
                      className="inline-flex items-center rounded-md bg-lime-50 px-2 py-0.5 text-[10px] font-medium text-lime-700"
                    >
                      {e}
                    </span>
                  ))}
                ...
                {p.tribunais.length > 0 && (
                  <div className="flex items-start gap-1.5">
                    <Scale className="h-3 w-3 text-slate-400 flex-shrink-0 mt-0.5" />
                    <div className="flex flex-wrap gap-1">
                      {p.tribunais.map((t) => (
                        <span key={t} className="inline-flex items-center rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-500">
                          {t}
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-auto">
                  <span className="text-xs text-slate-500">
                    A partir de <strong className="text-slate-800">{formatCurrency(p.valor_referencia)}</strong>
                  </span>
                  {enviado.has(p.id) ? (
                    ...
                    <Button
                      size="sm"
                      className="gap-1 bg-lime-500 hover:bg-lime-600 text-slate-900 font-semibold"
        ...
        >
          <div className="w-full max-w-md rounded-xl bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b border-slate-100 p-5">
              <div>
                <h2 className="text-base font-semibold text-slate-900">Enviar Proposta</h2>
                <p className="text-xs text-slate-500 mt-0.5">{modal.peritoNome}</p>
              </div>
              <button
                onClick={() => setModal(null)}
                className="ml-4 rounded-lg p-1 text-slate-400 hover:bg-slate-100 transition-colors"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="p-5 space-y-4">
              {demandaTitulo && (
                <div className="rounded-lg bg-lime-50 border border-lime-100 p-3">
                  <p className="text-xs text-lime-700 font-medium">Demanda vinculada</p>
                  <p className="text-xs text-lime-600 mt-0.5">{demandaTitulo}</p>
                </div>
              )}
              <div>
                ...
                <textarea
                  ...
                  className="w-full resize-none rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-lime-500 focus:outline-none focus:ring-1 focus:ring-lime-500"
                />
              </div>
            </div>
            <div className="flex gap-2 border-t border-slate-100 px-5 py-4">
              ...
              <Button
                size="sm"
                className="flex-1 gap-1 bg-lime-500 hover:bg-lime-600 text-slate-900 font-semibold"
